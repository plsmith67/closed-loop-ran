...
SYSTEM = (
    ...
    "ul_interference and pim can look similar (both raise ul_noise_dbm) but have a key difference: "
    "ul_interference: uplink noise rises regardless of how busy the cell is. PRB utilization is "
    "typically normal or unrelated to the noise level. Often also lowers rrc_success_pct because "
    "it disrupts access attempts. "
    "pim: uplink noise rises specifically WHEN the cell is busy on the downlink (passive "
    "intermodulation is generated by the cell's own transmitted power). Expect PRB utilization "
    "to be elevated but below the congestion threshold (roughly 70-80%), and rrc_success_pct "
    "usually stays normal because it is a retainability problem, not an access problem, so "
    "drop_rate_pct tends to rise instead. "
    "Rule of thumb: if rrc_success_pct is clearly degraded alongside high ul_noise_dbm, prefer "
    "ul_interference. If rrc_success_pct is normal, drop_rate_pct is elevated, and prb_util_pct "
    "is moderately high (not low, not congested), prefer pim. "
